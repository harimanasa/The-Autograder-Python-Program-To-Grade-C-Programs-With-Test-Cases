...
os.system("for d in sorted_dirs/*/; do cp main.cc \"$d\"; done")
...
for (dirname, dirs, files) in os.walk('.'):
   for filename in files:
	...
	#uncomment below to preprend header file in main:w	        
        insert(dirname+"/main.cc", "#include \""+filename+"\" \n")	
#print(COUNT)
...
for (dirname, dirs, files) in os.walk('.'):
   for filename in files:
       if filename.endswith('.cc') and filename != 'main.cc':
            ...
            os.system("g++ -c "+dirname+"/main.cc "+ thefile)
            os.system("g++ main.o "+object_name)
            os.system("./a.out >> "+dirname+"_output.txt")
            os.system("echo \" \"; echo \" ====================== \" >> FinalResults_differences_withIdeal.txt ")
            os.system("echo  Differences between Ideal_output.txt and" +dirname +"_output.txt  >> FinalResults_differences_withIdeal.txt")
            os.system("diff -w Ideal_output.txt "+dirname+"_output.txt >> FinalResults_differences_withIdeal.txt")
            os.remove('./a.out')
